...
function getSmartLayout() {
    let components = [
        ...
        // { id: 'projects', priority: 7, component: renderProjectsOverviewCard, width: 1, height: 1 }, // Removed as per user request
...
function recalculateStreak(habit) {
    ...
    const doneDates = habit.history
        ...
        .sort((a, b) => b.getTime() - a.getTime()); // Sort by time, descending

    // Use a Set to get unique dates
    const uniqueDates = [...new Set(doneDates.map(d => d.getTime()))].map(t => new Date(t));
    ...
    for (const date of uniqueDates) {
        const dayDifference = Math.floor((lastDate.getTime() - date.getTime()) / (1000 * 60 * 60 * 24));
        if (dayDifference <= 1) { // Check for today or a consecutive day
            streak++;
            lastDate = date;
        } else {
    ...
    // Final check for the case where the most recent entry isn't today or yesterday.
    const mostRecentEntry = uniqueDates[0];
    const dayDiffFromToday = Math.floor((today.getTime() - mostRecentEntry.getTime()) / (1000 * 60 * 60 * 24));
    ...
    function animateRockets() {
        ...
        threeJS.objects.rockets.forEach(rocket => {
            ...
            if(rocket.position.y > 100) {
                ...
                rocket.position.x = THREE.MathUtils.randFloatSpread(150);
                rocket.position.z = THREE.MathUtils.randFloatSpread(50) - 50;
                rocket.speed = Math.random() * 0.2 + 0.1; // New speed for new launch
            }
        });
...
window.copyUserId = () => {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(userId).then(() => {
            ...
            copyToClipboardFallback(userId, copyBtn);
        });
    } else {
